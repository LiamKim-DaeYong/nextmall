# ADR-005: 모듈 간 의존성 원칙 및 test-support 역할 정의

> 순환 의존성 제거 및 명확한 모듈 계층 구조 확립. test-support는 테스트 인프라 제공자, common은 순수 비즈니스 로직
...
프로젝트 초기에는 모듈 간 의존성 방향에 대한 명확한 원칙이 없었고, 편의성을 위해 자유롭게 의존성을 추가하다 보니 순환 의존성 문제가 발생했다.
...
### 3. 명확한 계층 구조 정의 (선택)
- 장점: 의존성 방향 명확, 순환 의존성 원천 차단, 확장성 우수
...
### 얻는 것
- 명확한 의존성 방향으로 새로운 모듈 추가 시 혼란 감소
- 순환 의존성 제거로 빌드 안정성 향상
- common 모듈 순수성 확보로 프로덕션 코드와 테스트 코드의 경계 명확
- 테스트 전략 명확화 (단위 테스트 vs 통합 테스트)
...
- 실제 통합 테스트는 상위 계층(modules/services)에서 수행하므로 문제없음
