...
- Chores
  - 자동 변경 로그 생성 워크플로우 추가(PR 병합 시 요약을 추출해 docs/changelog.md 상단에 새 항목을 자동 삽입 및 커밋)
- Documentation
  - v1.0.5 변경 로그 항목 추가: SonarCloud 기반 코드품질 통합, CI/CD 파이프라인 개선(캐시·의존성 그래프·커버리지 검증 등), 테스트 환경 및 CI 안전성 개선
