...
본 문서는 Kafka 기반 이벤트 설계의 최소 기준을 정의한다.
포트폴리오/학습 목적을 고려해 최소 범위로 시작하되, 실무 설계 의도와 확장성을 드러내는 것을 목표로 한다.

---

## 1. 목표
- 서비스 간 비동기 계약을 명확히 정의한다
...
## 3. 이벤트 공통 규칙
- 이벤트는 봉투(Envelope) + 페이로드(Payload) 구조를 가진다
- 모든 이벤트는 고유 eventId를 가진다 (UUID 권장)
- 발생 시각 occurredAt은 RFC 3339 UTC로 기록한다
- 이벤트 타입은 명시적 문자열로 관리한다
- 이벤트는 하위 호환 변경만 허용한다 (새 필드 추가만 허용)
...
- 정책 값은 `circuit-breaker.md` 기준을 따른다
